...
public class ListHands_Test {
	...
	public void ListHoldEm() {
		...
		//This test fails because the total possible hands value for texas holdem in the DB is wrong....
		...
		GameRuleDomainModel rle = hs.get(strRuleName);
		System.out.println(rle.getRULEID());
		ArrayList<GameRuleCardsDomainModel> picks = GameRuleCardsBLL.getCardsRules(rle.getRULEID());
		
		GamePlay gme = new GamePlay(rle, picks);
		...
		System.out.println(gme.getRule().getPOSSIBLEHANDCOMBINATIONS());
