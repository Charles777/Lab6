...
public class Action {
	...
	public Action(GamePlay gme, int iCardDrawnPlayer, int iCardDrawnCommon, int iDrawCount) {
		...
		if (iCardDrawnPlayer + iCardDrawnCommon == gme.getRule().getCOMMUNITYCARDSMAX()+gme.getRule().getPLAYERNUMBEROFCARDS())
		{
			this.bEvaluateHand = true;
		}
		this.setiCardDrawn(gme.getRulePicks().get(iDrawCount-1).getNBROFCARDS());
	}
